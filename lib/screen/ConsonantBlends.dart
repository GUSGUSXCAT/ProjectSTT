...
class ConsonantBlendscreen extends StatefulWidget {
  final String type;
  final String set;

  const ConsonantBlendscreen({
    super.key,
    required this.type,
    required this.set,
  });
  ...
  @override
  State<ConsonantBlendscreen> createState() => _ConsonantBlendscreenState();
}

class _ConsonantBlendscreenState extends State<ConsonantBlendscreen> {
  ...
  Future<void> saveQuizResult() async {
    try {
      ...
      await _firestore.collection('quiz_results').add({
        ...
        "category": "ควบกลํ้า (ประเภท ${widget.type} ชุดที่ ${widget.set})",
  ...
  Widget build(BuildContext context) {
    ...
    if (words.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: Text(
            "ควบกลํ้า (ประเภท ${widget.type} ชุดที่ ${widget.set})",
            style: GoogleFonts.mali(),
          ),
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            Expanded(
              child: Text(
                "ควบกลํ้า (${widget.type} ชุด ${widget.set})",
                style: GoogleFonts.mali(fontSize: 18),
              ),
            ),
            Text(
              'ข้อ ${currentIndex + 1}/${words.length}',
              style: GoogleFonts.mali(fontSize: 18),
            ),
          ],
        ),
                                    ...
                                    Text(
                                      isCorrect
                                          ...
                                          : (attempts >= _maxAttempts
                                              ? "หมดโอกาสแล้ว ลองพูดใหม่อีกครั้ง"
