...
class ShortSentencesPracticeScreen extends StatefulWidget {
  final String set;

  const ShortSentencesPracticeScreen({super.key, required this.set});

  @override
  State<ShortSentencesPracticeScreen> createState() => _ShortSentencesPracticeScreenState();
}

class _ShortSentencesPracticeScreenState extends State<ShortSentencesPracticeScreen> {
  ...
  Future<void> saveQuizResult() async {
    try {
      ...
      await _firestore.collection('quiz_results').add({
        ...
        "category": "อ่านประโยคสั้น ${widget.set}",
                  ...
                  ConstrainedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final att = w['attempt'] ?? 1;
                        return Text(
                          'ข้อ ${w['index']}: พูดว่า "${w['recognized']}" | ควรพูด "${w['correct']}" (ครั้งที่ $att)${w['note'] != null ? ' - ${w['note']}' : ''}',
  ...
  Widget build(BuildContext context) {
    ...
    if (words.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: Text(
            'ฝึกอ่านประโยคสั้น ชุดที่ ${widget.set}',
            style: GoogleFonts.mali(),
          ),
                  ...
                  child: IntrinsicHeight(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            '🐈⋆୨୧˚ ให้นักเรียนอ่านประโยคที่นักเรียนเห็นบนจอ',
