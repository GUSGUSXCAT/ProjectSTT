...
class ReadingWordsScreen extends StatefulWidget {
  ...
  const ReadingWordsScreen({super.key, required this.set});
...
class _ReadingWordsScreenState extends State<ReadingWordsScreen> {
  ...
  Future<void> saveQuizResult() async {
    try {
      ...
      await _firestore.collection('quiz_results').add({
        ...
        "category": "ฝึกอ่านคำ ป.1 ชุดที่ ${widget.set}",
  ...
  // ⭐ อ่านออกเสียงเฉพาะคำแรก (currentIndex == 0)
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton(
                        // ⭐ กดปิดแล้วกลับแดชบอร์ดเลย (pop 3 ครั้ง)
                        onPressed: () {
  ...
  void _resetQuiz() {
    ...
    speakCurrentTts(); // อ่านคำแรกอีกครั้งเมื่อทำใหม่
  }
  ...
  Future<void> _moveToNextQuestion() async {
    if (!isReviewingSkipped) {
      if (currentIndex < words.length - 1) {
        ...
        // ⭐ ไม่เรียก speakCurrentTts() เพราะอ่านแค่คำแรก
        return;
      }
  ...
  void _checkSpeech() async {
    ...
    final expected = words[currentIndex]['check'].toString().trim();
    ...
    setState(() {
      ...
      if (wasCorrect) {
        // ⭐ คิดคะแนน: ถ้าตอบถูกภายใน 3 ครั้ง (หรือ 1 ครั้งถ้าเป็นข้อข้าม) = +1
    ...
    if (!wasCorrect && nextAttempts >= maxAttempts) {
      ...
      await _tts.speak("คำที่ถูกต้องคือ $expected");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (words.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: Text('ฝึกอ่านคำ ป.1 ชุดที่ ${widget.set}',
              style: GoogleFonts.mali()),
          backgroundColor: Colors.green.shade100,
        ),
        body: Center(
          child: Text(
            'ยังไม่มีข้อมูลในชุดนี้',
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(
          ...
          children: [
            Text('ฝึกอ่านคำ ป.1 ชุดที่ ${widget.set}',
                style: GoogleFonts.mali(fontSize: 18)),
            Text(
                  ...
                  child: IntrinsicHeight(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            '🐈⋆୨୧˚ ให้นักเรียนอ่านคำที่เห็นบนจอโดยไม่ต้องสะกดคำ',
