...
class TonePlacementScreen extends StatefulWidget {
  const TonePlacementScreen({super.key});

  @override
  State<TonePlacementScreen> createState() =>
      _TonePlacementScreenState();
}

class _TonePlacementScreenState extends State<TonePlacementScreen> {
  final FlutterTts _tts = FlutterTts();
  final stt.SpeechToText _speech = stt.SpeechToText();
  List<DocumentSnapshot> TonePlace = [];
  int currentIndex = 0;
  bool isLoading = true;
  ...
  Future<void> fetchConsonants() async {
    final snapshot =
        await FirebaseFirestore.instance
            ...
            .doc('g_1')
            .collection('toneplacement')
            .orderBy(FieldPath.documentId)
            .get();

    setState(() {
      TonePlace = snapshot.docs;
      isLoading = false;
    });
  ...
  Future<void> speakCurrentTts() async {
    final ttsText = TonePlace[currentIndex]['tts_text'];
    await _tts.setLanguage("th-TH");
    await _tts.speak(ttsText);
  }

  void _next() {
    if (currentIndex < TonePlace.length - 1) {
      setState(() {
        currentIndex++;
        recognizedText = "";
        isCorrect = false;
  ...
  void _checkSpeech() {
    ...
    setState(() {
      isListening = false;
      final expected = TonePlace[currentIndex]['check'].trim();
  ...
  Widget build(BuildContext context) {
    ...
    final data = TonePlace[currentIndex].data() as Map<String, dynamic>;

    return Scaffold(
  appBar: AppBar(
    title: Text('ฝึกอ่านการวางวรรณยุกต์ ป.1', style: GoogleFonts.mali()),
    centerTitle: true,
